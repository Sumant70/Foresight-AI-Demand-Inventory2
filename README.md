...
The platform provides an executive Single Page Application (SPA) dashboard powered by a lightweight, zero-dependency Python HTTP server running locally on `http://localhost:5000`.

---

## 2. Technologies Used

- **Backend & Core Engine:** Python 3 (Standard Library: `http.server`, `json`, `csv`, `math`, `sqlite3`, `hashlib`, `hmac`, `urllib`, `pathlib`, `unittest`)
- **Frontend Architecture:** Pure Vanilla JavaScript (ES6+), HTML5 Semantic Markup, CSS3 Design System
- **Visualizations:** Pure HTML5 Canvas & CSS responsive data charts (Zero external charting libraries, Zero SVG)
- **Data Storage:** Flat-file tabular CSV format (`data/raw/` as immutable source of truth) and embedded SQLite (`users.db`)
- **Third-Party Dependencies:** **ZERO** (No Node.js, no npm, no React/Next.js, no pip packages, no external CDNs)
...
- **Python Runtime:** Python 3.9 or newer (tested on Python 3.10, 3.11, 3.12, 3.13)
- **Web Browser:** Any modern web browser (Google Chrome, Microsoft Edge, Mozilla Firefox, Apple Safari)
- **Network Access:** Fully offline capable; no internet connection required
...
1. **Extract the Project Archive:**
   Extract `project-complete.zip` to your desired working folder (e.g. `C:\Projects\foresight-ai`).
...
3. **Install Dependencies:**
   ...
   # All modules run on the Python standard library.
...
The project works out-of-the-box with default settings. To customize runtime behavior:
...
| :--- | :--- | :--- |
| `PORT` | `5000` | Local HTTP server port (`http://localhost:5000`) |
| `HOST` | `0.0.0.0` | Bind IP address (`0.0.0.0` for all interfaces, `127.0.0.1` for local only) |
| `ENVIRONMENT` | `production` | Operational environment mode (`development` or `production`) |
| `FORESIGHT_SESSION_HOURS` | `8` | Session duration in hours |
| `FORESIGHT_IDLE_MINUTES` | `30` | Inactivity timeout before session expiration |
| `FORESIGHT_SECURE_COOKIE` | `false` | Enable when running behind an HTTPS / TLS reverse proxy |

> [!NOTE]
> NEVER commit `.env` files containing sensitive passwords or keys to version control. The included `.env.example` contains safe placeholder defaults.
...
==================================================================
  ...
  Access Mode: Direct Public Dashboard (Authentication Bypassed)
==================================================================
```
...
To verify all 49 automated unit tests across data processing, forecasting models, inventory calculations, ABC/XYZ segmentation, and REST endpoints:
...
- **Source of Truth:** Raw business datasets located in `data/raw/` (`sales_daily.csv`, `sku_master.csv`, `inventory_snapshots.csv`, `calendar.csv`) are strictly treated as read-only.
- **Direct Public Access:** The web dashboard is configured for direct public access—no login or password is required to inspect the 8 analytical domains.
- **Zero-Dependency Guarantee:** The platform requires zero npm commands, zero external JavaScript bundles, zero CSS frameworks, and zero pip installations. It is 100% self-contained and portable.
- **Strictly Zero SVG:** In compliance with strict enterprise rendering constraints, all icons and charts use pure CSS and HTML5 Canvas.
...
```
FORESIGHT-AI/
├── app.py                     # High-performance Python http.server, REST API dispatcher, Security Middleware
├── create_admin.py            # CLI Administrator account setup & credential reset utility
├── users.db                   # SQLite database for users, sessions, password resets, and audit logs
├── data/
...
│   └── data_quality_report.json
├── src/
│   ├── security/              # Enterprise Security & Authentication Infrastructure
│   │   ├── __init__.py
│   │   ├── database.py        # SQLite users.db manager (users, sessions, audit_logs)
│   │   ├── auth.py            # PBKDF2-HMAC-SHA256 hasher, policy validator, brute-force mitigation
│   │   ├── session.py         # Server-side sessions, idle timeout, max age, HttpOnly cookie builder
│   │   ├── csrf.py            # Session-bound anti-CSRF token protection
│   │   └── rbac.py            # Role-Based Access Control policies (ADMIN, ANALYST, VIEWER)
│   ├── data_processing/
...
├── templates/
│   ├── index.html             # 10-tab SPA enterprise executive interface (zero SVG)
│   └── login.html             # 2-column B2B SaaS authentication interface (zero SVG)
├── static/
│   ├── style.css              # Dark slate & light canvas executive CSS design system
│   └── app.js                 # Pure Vanilla JS router, Canvas chart renderer, auth controller
├── tests/                     # Comprehensive automated unit & integration test suite (49 tests)
...
Run the complete 49-test automated test suite using Python's built-in `unittest` runner:
...
## 15. Enterprise Security & Authentication Architecture

FORESIGHT AI is fortified with an enterprise-grade security layer designed strictly with Python standard library and vanilla web standards (zero external dependencies, zero NPM, zero external CDNs, zero SVG).

### 1. Default Master Administrator Credentials
- **Username:** `admin`
- **Email:** `admin@foresight.ai`
- **Master Password:** `ForesightAdmin2026!`
- **Role:** `ADMIN`

### 2. Cryptographic Password Protection
- **Cipher:** `PBKDF2-HMAC-SHA256` executed with **100,000 rounds** and **32-byte cryptographically secure random salt** per user (`secrets.token_bytes(32)`).
- **Constant-Time Verification:** Employs `hmac.compare_digest` to prevent side-channel timing attacks.
...
- Failed login attempts are tracked per user in `users.db`.
- Reaching **5 consecutive failed attempts** immediately locks the account for **15 minutes**.
- Generic, timing-safe error messaging never leaks whether the username or password specifically was incorrect.
- Administrators can instantly unlock accounts via the User Management panel or CLI.

### 4. Server-Side Session Management
- **Token Generation:** 32-byte cryptographic tokens (`secrets.token_urlsafe(32)`).
- **Session Duration:** 8-hour maximum lifetime (configurable; 7-day with "Remember Me").
- **Idle Timeout:** 30 minutes of client inactivity revokes the session automatically.
- **Cookie Policy:** Delivered via `Set-Cookie` with `HttpOnly; SameSite=Lax; Path=/`.

### 5. Anti-CSRF Token Enforcement
- State-changing HTTP methods (`POST`, `PUT`, `DELETE`) require a cryptographically random session-bound CSRF token via `X-CSRF-Token` header.
- Mutating requests with missing or mismatched tokens are rejected with `403 Forbidden` and logged.

### 6. Role-Based Access Control (RBAC) Matrix

| Platform Capability | ADMIN | ANALYST | VIEWER |
| :--- | :---: | :---: | :---: |
| **Executive Overview & Insights** | Full Access | Full Access | Read-Only |
| **Demand Forecasting (7/14/30d)** | Full Access | Full Access | Restricted |
| **Inventory Health Matrix** | Full Access | Full Access | Restricted |
| **Stockout Risk Projections** | Full Access | Full Access | Restricted |
| **Reorder Engine & Simulation** | Full Access | Full Access | Restricted |
| **Product Analytics & 9-Box ABC/XYZ** | Full Access | Full Access | Read-Only |
| **Data Quality & Schema Profiling** | Full Access | Full Access | Restricted |
| **Model Benchmark Tournament** | Full Access | Full Access | Restricted |
| **CSV Tabular Exports** | Full Access | Full Access | Read-Only |
| **User Management (Provision/Roles/Status)**| Full Access | Restricted | Restricted |
| **Security Metrics & Audit Trail Ledger** | Full Access | Restricted | Restricted |

### 7. HTTP Security Headers
All server responses enforce defense-in-depth headers:
...
### 8. CLI Administration Utility (`create_admin.py`)
Administrators can initialize or reset master credentials securely from terminal without exposing passwords in command histories:

```powershell
# Interactive password entry with masked terminal input:
python create_admin.py --username admin --email admin@foresight.ai

# Password override / account unlock:
python create_admin.py --username admin --email admin@foresight.ai --password "NewSecurePassword2026!" --reset
```

---

## 16. How to Use the Dashboard

The platform features a 10-tab executive navigation sidebar:
...
9. **User Management (Admin Only):** Provision team member accounts, adjust RBAC roles, deactivate/activate users, unlock brute-force locked accounts, and reset credentials.
10. **Security & Audit (Admin Only):** Monitor active session counts, 24-hour brute-force blocks, cryptographic verification status, and tamper-evident audit log ledger.
